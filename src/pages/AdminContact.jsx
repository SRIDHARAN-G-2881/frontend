...
const ContactList = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/server/contact/displaycontact`, {credentials: 'include',});
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${BACKEND_URL}/server/contact/deletecontact/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
